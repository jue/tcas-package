import { gQ as _, gR as w, dh as D } from "./index-HxXrdrYt.js";
import { c as x } from "./observers-irFNFMFU.js";
/*!
...
function ne(e) {
  return !e || !Z(e) ? "" : f(e, (t) => {
    let s = !1;
    const n = t.split("").filter((r, a) => r.match(/\./g) && !s ? (s = !0, !0) : r.match(/\-/g) && a === 0 ? !0 : I.includes(r)).join("");
    return $(n) ? new i(n).toString() : "";
  });
}
const N = /^([-0])0+(?=\d)/, j = /(?!^\.)\.$/, z = /(?!^-)-/g, k = /^-\b0\b\.?0*$/, P = /0*$/, re = (e) => f(e, (t) => {
  const s = t.replace(z, "").replace(j, "").replace(N, "$1");
  return $(s) ? k.test(s) ? s : H(s) : t;
});
function H(e) {
...
function U(e) {
  ...
  if (Number.isSafeInteger(s))
    return `${s}`;
  const n = e.charAt(0) === "-", r = +t[1], a = t[0].split("."), l = (n ? a[0].substring(1) : a[0]) || "", S = a[1] || "", M = (c, u) => {
    const g = Math.abs(u) - c.length, p = g > 0 ? `${"0".repeat(g)}${c}` : c;
    return `${p.slice(0, u)}.${p.slice(u)}`;
  }, A = (c, u) => {
    const g = u > c.length ? `${c}${"0".repeat(u - c.length)}` : c;
    return `${g.slice(0, u)}.${g.slice(u)}`;
  }, y = r > 0 ? `${l}${A(S, r)}` : `${M(l, r)}${S}`;
  return `${n ? "-" : ""}${y.charAt(0) === "." ? "0" : ""}${y.replace(R, "").replace(N, "")}`;
}
function Z(e) {
...
const o = "en", G = [
  ...
  "thai",
  "tibt"
], F = (e) => K.includes(e), b = new Intl.NumberFormat().resolvedOptions().numberingSystem, O = b === "arab" || !F(b) ? "latn" : b, J = (e) => F(e) ? e : O;
function L(e, t = "cldr") {
  const s = t === "cldr" ? B : G;
  return e ? s.includes(e) ? e : (e = e.toLowerCase(), e === "nb" ? "no" : t === "t9n" && e === "pt" ? "pt-BR" : (e.includes("-") && (e = e.replace(/(\w+)-(\w+)/, (n, r, a) => `${r}-${a.toUpperCase()}`), s.includes(e) || (e = e.split("-")[0])), e === "zh" ? "zh-CN" : s.includes(e) ? e : (console.warn(`Translations for the "${e}" locale are not available and will fall back to the default, English (en).`), o))) : o;
}
const m = /* @__PURE__ */ new Set();
function ae(e) {
  W(e), m.size === 0 && (d == null || d.observe(document.documentElement, {
    attributes: !0,
...
function W(e) {
  e.effectiveLocale = Q(e);
}
function ce(e) {
  m.delete(e), m.size === 0 && d.disconnect();
}
const d = x("mutation", (e) => {
  e.forEach((t) => {
    const s = t.target;
    m.forEach((n) => {
      if (!_(s, n.el))
      ...
      a.hasAttribute("lang") && l === "" ? o : l;
    });
  });
});
function Q(e) {
  var t;
  return e.el.lang || ((t = w(e.el, "[lang]")) == null ? void 0 : t.lang) || document.documentElement.lang || o;
}
class X {
  constructor() {
    this.delocalize = (t) => (
      ...
      this._numberFormatOptions ? f(t, (s) => s.replace(new RegExp(`[${this._minusSign}]`, "g"), "-").replace(new RegExp(`[${this._group}]`, "g"), "").replace(new RegExp(`[${this._decimal}]`, "g"), ".").replace(new RegExp(`[${this._digits.join("")}]`, "g"), this._getDigitIndex)) : t
    ), this.localize = (t) => this._numberFormatOptions ? f(t, (s) => $(s.trim()) ? new i(s.trim()).format(this).replace(new RegExp(`[${this._actualGroup}]`, "g"), this._group) : s) : t;
  }
  get group() {
    return this._group;
  }
  ...
  set numberFormatOptions(t) {
    if (t.locale = L(t == null ? void 0 : t.locale), t.numberingSystem = J(t == null ? void 0 : t.numberingSystem), // No need to create the formatter if `locale` and `numberingSystem`
    // are the default values and `numberFormatOptions` has not been set
    !this._numberFormatOptions && t.locale === o && t.numberingSystem === O && // don't skip initialization if any options besides locale/numberingSystem are set
...
async function q(e, t) {
  const s = `${t}_${e}`;
  return h[s] || (h[s] = fetch(D(`./assets/${t}/t9n/messages_${e}.json`)).then((n) => (n.ok || E(), n.json())).catch(() => E())), h[s];
}
function E() {
...
async function oe(e) {
  e.defaultMessages = await C(e, e.effectiveLocale), v(e);
}
...
async function le(e, t) {
  e.defaultMessages = await C(e, t), v(e);
}
function ge(e) {
