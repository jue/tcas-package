...
class Pt {
  ...
  bidiTransform(o, u, a) {
    if (this.sourceToTarget = [], this.targetToSource = [], !o)
      return "";
    if (Ot(this.sourceToTarget, this.targetToSource, o.length), !this.checkParameters(u, a))
      return o;
    u = this.inputFormat, a = this.outputFormat;
    let f = o;
    const c = It, L = H(u.charAt(1)), A = H(a.charAt(1)), g = (u.charAt(0) === "I" ? "L" : u.charAt(0)) + L, d = (a.charAt(0) === "I" ? "L" : a.charAt(0)) + A, U = u.charAt(2) + a.charAt(2);
    c.defInFormat = g, c.defOutFormat = d, c.defSwap = U;
    const w = E(o, g, d, U, c);
    let N = !1;
    return a.charAt(1) === "R" ? N = !0 : a.charAt(1) !== "C" && a.charAt(1) !== "D" || (N = this.checkContextual(w) === "rtl"), this.sourceToTarget = F, this.targetToSource = xt(this.sourceToTarget), P = this.targetToSource, f = u.charAt(3) === a.charAt(3) ? w : a.charAt(3) === "S" ? Bt(N, w, !0) : Rt(w, N, !0), this.sourceToTarget = F, this.targetToSource = P, this.levels = y, f;
  }
...
function E(i, o, u, a, f) {
  const c = dt(i, { inFormat: o, outFormat: u, swap: a }, f);
  if (c.inFormat === c.outFormat)
    return i;
  o = c.inFormat, u = c.outFormat, a = c.swap;
  const L = o.substring(0, 1), A = o.substring(1, 4), g = u.substring(0, 1), d = u.substring(1, 4);
  if (f.inFormat = o, f.outFormat = u, f.swap = a, L === "L" && u === "VLTR") {
    if (A === "LTR")
      return f.dir = C, S(i, f);
    if (A === "RTL")
      return f.dir = O, S(i, f);
  }
  if (L === "V" && g === "V")
    return f.dir = A === "RTL" ? O : C, Y(i, f);
  if (L === "L" && u === "VRTL")
    return A === "LTR" ? (f.dir = C, i = S(i, f)) : (f.dir = O, i = S(i, f)), Y(i);
  if (o === "VLTR" && u === "LLTR")
    return f.dir = C, S(i, f);
  if (L === "V" && g === "L" && A !== d)
    return i = Y(i), A === "RTL" ? E(i, "LLTR", "VLTR", a, f) : E(i, "LRTL", "VRTL", a, f);
  if (o === "VRTL" && u === "LRTL")
    return E(i, "LRTL", "VRTL", a, f);
  if (L === "L" && g === "L") {
    const U = f.swap;
    return f.swap = U.substr(0, 1) + "N", A === "RTL" ? (f.dir = O, i = S(i, f), f.swap = "N" + U.substr(1, 2), f.dir = C, i = S(i, f)) : (f.dir = C, i = S(i, f), f.swap = "N" + U.substr(1, 2), i = E(i, "VLTR", "LRTL", f.swap, f)), i;
  }
  return i;
}
function dt(i, o, u) {
  if (o.inFormat === void 0 && (o.inFormat = u.defInFormat), o.outFormat === void 0 && (o.outFormat = u.defOutFormat), o.swap === void 0 && (o.swap = u.defSwap), o.inFormat === o.outFormat)
    return o;
...
function Bt(i, o, u) {
  if (o.length === 0)
    return "";
  i === void 0 && (i = !0), u === void 0 && (u = !0);
  const a = (o = String(o)).split("");
  let f = 0, c = 1, L = a.length;
  i || (f = a.length - 1, c = -1, L = 1);
  const A = Ut(a, f, c, L, u);
  let g = "";
  for (let d = 0; d < a.length; d++)
    u && _t(A, A.length, d) > -1 ? (X(P, d, !i, -1), F.splice(d, 1)) : g += a[d];
  return g;
}
function Ut(i, o, u, a, f) {
  let c = 0;
  const L = [];
  let A = 0;
  for (let g = o; g * u < a; g += u)
    if (Q(i[g]) || x(i[g])) {
      if (i[g] === "ل" && St(i, g + u, u, a)) {
        i[g] = Et(i[g + u], c === 0 ? $ : tt), g += u, Vt(i, g, u, a), f && (L[A] = g, A++), c = 0;
        continue;
      }
      const d = i[g];
      c === 1 ? i[g] = Z(i, g + u, u, a) ? Ct(i[g]) : W(i[g], et) : Z(i, g + u, u, a) === !0 ? i[g] = W(i[g], ot) : i[g] = W(i[g], rt), x(d) || (c = 1), vt(d) === !0 && (c = 0);
    } else
      c = 0;
  return L;
}
...
function Rt(i, o, u) {
  ...
  for (let c = 0; c < i.length; c++) {
    ...
    if (f[c] >= "ﹰ" && f[c] < "\uFEFF") {
      ...
      f[c] >= "ﻵ" && f[c] <= "ﻼ" ? (o ? (c > 0 && u && f[c - 1] === " " ? a = a.substring(0, a.length - 1) + "ل" : (a += "ل", L = !0), a += V[(A - 65269) / 2]) : (a += V[(A - 65269) / 2], a += "ل", c + 1 < i.length && u && f[c + 1] === " " ? c++ : L = !0), L && (X(P, c, !0, 1), F.splice(c, 0, F[c]))) : a += it[A - 65136];
    } else
      a += f[c];
  }
  return a;
...
function S(i, o) {
  const u = i.split(""), a = [];
  return K(u, a, o), bt(u, a, o), G(2, u, a, o), G(1, u, a, o), y = a, u.join("");
}
function K(i, o, u) {
  const a = i.length, f = u.dir ? at : st;
  let c = 0, L = -1;
  const A = [], g = [];
  ...
  for (let d = 0; d < a; d++) {
    ...
    A[d] === _ && (o[d] = 0), u.hiLevel |= M;
  }
  u.hasUbatS && Ft(A, o, a, u);
}
function Ft(i, o, u, a) {
  for (let f = 0; f < u; f++)
    if (i[f] === j) {
      o[f] = a.dir;
      for (let c = f - 1; c >= 0 && i[c] === m; c--)
        o[c] = a.dir;
    }
}
function bt(i, o, u) {
...
function G(i, o, u, a) {
  if (a.hiLevel < i)
    return;
  if (i === 1 && a.dir === O && !a.hasUbatB)
    return o.reverse(), void F.reverse();
  const f = o.length;
  let c, L, A, g, d, U = 0;
  for (; U < f; ) {
    if (u[U] >= i) {
      for (c = U + 1; c < f && u[c] >= i; )
        c++;
      for (L = U, A = c - 1; L < A; L++, A--)
        g = o[L], o[L] = o[A], o[A] = g, d = F[L], F[L] = F[A], F[A] = d;
      U = c;
    }
    U++;
  }
}
function wt(i, o, u, a, f) {
  const c = o[a];
  return { UBAT_L: () => (f.lastArabic = !1, e), UBAT_R: () => (f.lastArabic = !1, s), UBAT_ON: () => t, UBAT_AN: () => B, UBAT_EN: () => f.lastArabic ? B : h, UBAT_AL: () => (f.lastArabic = !0, f.hasUbatAl = !0, s), UBAT_WS: () => t, UBAT_CS: () => {
    let L, A;
    return a < 1 || a + 1 >= o.length || (L = u[a - 1]) !== h && L !== B || (A = o[a + 1]) !== h && A !== B ? t : (f.lastArabic && (A = B), A === L ? A : t);
  }, UBAT_ES: () => (a > 0 ? u[a - 1] : _) === h && a + 1 < o.length && o[a + 1] === h ? h : t, UBAT_ET: () => {
    if (a > 0 && u[a - 1] === h)
      return h;
    if (f.lastArabic)
      return t;
...
function Nt(i) {
  let o, u = 0, a = I.length - 1;
  for (; u <= a; )
    if (o = Math.floor((u + a) / 2), i < I[o][0])
      a = o - 1;
    else {
      if (!(i > I[o][0]))
        return I[o][1];
...
let F = [], P = [], y = [];
const It = { dir: 0, defInFormat: "LLTR", defoutFormat: "VLTR", defSwap: "YN", inFormat: "LLTR", outFormat: "VLTR", swap: "YN", hiLevel: 0, lastArabic: !1, hasUbatAl: !1, hasBlockSep: !1, hasSegSep: !1, defOutFormat: "" }, jt = 5, kt = 6, C = 0, O = 1, J = /^[(I|V)][(L|R|C|D)][(Y|N)][(S|N)][N]$/, Dt = /[\u0591-\u06ff\ufb1d-\ufefc]/;
export {
  Pt as C
};
