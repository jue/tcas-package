import { z as o, bq as h } from "./index-O2RTvw_o.js";
import { h as a } from "./PooledRBush-SNj0w-D4.js";
...
class B {
  constructor() {
    this._indexInvalid = !1, this._boundsToLoad = [], this._boundsById = /* @__PURE__ */ new Map(), this._idByBounds = /* @__PURE__ */ new Map(), this._index = new a(9, o("geoscene-csp-restrictions") ? (s) => ({ minX: s[0], minY: s[1], maxX: s[2], maxY: s[3] }) : ["[0]", "[1]", "[2]", "[3]"]), this._loadIndex = () => {
      if (this._indexInvalid) {
        ...
        this._boundsToLoad.length && (this._index.load(Array.from(new Set(this._boundsToLoad.filter((s) => this._idByBounds.has(s))))), this._boundsToLoad.length = 0);
    };
  }
  get fullBounds() {
    if (!this._boundsById.size)
      return null;
    const s = h();
    for (const d of this._boundsById.values())
      d && (s[0] = Math.min(d[0], s[0]), s[1] = Math.min(d[1], s[1]), s[2] = Math.max(d[2], s[2]), s[3] = Math.max(d[3], s[3]));
    return s;
  }
  get valid() {
