const p = { Base64: 0, Hex: 1, String: 2, Raw: 3 }, x = 8, d = (1 << x) - 1;
function l(o, s) {
  const t = (65535 & o) + (65535 & s);
  return (o >> 16) + (s >> 16) + (t >> 16) << 16 | 65535 & t;
}
function C(o) {
  const s = [];
  for (let t = 0, r = o.length * x; t < r; t += x)
    s[t >> 5] |= (o.charCodeAt(t / x) & d) << t % 32;
  return s;
}
function H(o) {
  const s = [];
  for (let t = 0, r = 32 * o.length; t < r; t += x)
    s.push(String.fromCharCode(o[t >> 5] >>> t % 32 & d));
  return s.join("");
}
function S(o) {
  const s = "0123456789abcdef", t = [];
  for (let r = 0, e = 4 * o.length; r < e; r++)
    t.push(s.charAt(o[r >> 2] >> r % 4 * 8 + 4 & 15) + s.charAt(o[r >> 2] >> r % 4 * 8 & 15));
  return t.join("");
}
function R(o) {
  const s = "=", t = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", r = [];
  for (let e = 0, u = 4 * o.length; e < u; e += 3) {
    const c = (o[e >> 2] >> e % 4 * 8 & 255) << 16 | (o[e + 1 >> 2] >> (e + 1) % 4 * 8 & 255) << 8 | o[e + 2 >> 2] >> (e + 2) % 4 * 8 & 255;
    for (let g = 0; g < 4; g++)
      8 * e + 6 * g > 32 * o.length ? r.push(s) : r.push(t.charAt(c >> 6 * (3 - g) & 63));
  }
  return r.join("");
}
function b(o, s) {
...
function A(o, s, t, r, e, u) {
  return l(b(l(l(s, o), l(r, u)), e), t);
}
function f(o, s, t, r, e, u, c) {
  return A(s & t | ~s & r, o, s, e, u, c);
}
function i(o, s, t, r, e, u, c) {
  return A(s & r | t & ~r, o, s, e, u, c);
}
function h(o, s, t, r, e, u, c) {
  return A(s ^ t ^ r, o, s, e, u, c);
}
function a(o, s, t, r, e, u, c) {
  return A(t ^ (s | ~r), o, s, e, u, c);
}
function m(o, s) {
  o[s >> 5] |= 128 << s % 32, o[14 + (s + 64 >>> 9 << 4)] = s;
  let t = 1732584193, r = -271733879, e = -1732584194, u = 271733878;
  for (let c = 0; c < o.length; c += 16) {
    const g = t, j = r, w = e, B = u;
...
function k(o, s = p.Hex) {
  const t = s || p.Base64, r = m(C(o), o.length * x);
  switch (t) {
    case p.Raw:
      return r;
    case p.Hex:
