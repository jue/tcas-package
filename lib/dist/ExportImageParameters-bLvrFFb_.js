import { b0 as u, b1 as p, a, b as l, b2 as b, c as d, b3 as g } from "./index-HxXrdrYt.js";
import { n as m } from "./floorFilterUtils-EbR9s7nP.js";
import { n as f } from "./sublayerUtils-HD1T8Okk.js";
const S = { visible: "visibleSublayers", definitionExpression: "layerDefs", labelingInfo: "hasDynamicLayers", labelsVisible: "hasDynamicLayers", opacity: "hasDynamicLayers", minScale: "visibleSublayers", maxScale: "visibleSublayers", renderer: "hasDynamicLayers", source: "hasDynamicLayers" };
let t = class extends u(g) {
  ...
  get dynamicLayers() {
    ...
    const e = this.visibleSublayers.map((s) => {
      const n = m(this.floors, s);
      return s.toExportImageJSON(n);
    });
    return e.length ? JSON.stringify(e) : null;
  }
  get hasDynamicLayers() {
    return this.layer && f(this.visibleSublayers, this.layer.serviceSublayers, this.layer.gdbVersion);
  }
  set layer(e) {
    this._get("layer") !== e && (this._set("layer", e), this.handles.remove("layer"), e && this.handles.add([e.allSublayers.on("change", () => this.notifyChange("visibleSublayers")), e.on("sublayer-update", (s) => this.notifyChange(S[s.propertyName]))], "layer"));
  }
  get layers() {
  ...
  get layerDefs() {
    var n;
    const e = !!((n = this.floors) != null && n.length), s = this.visibleSublayers.filter((i) => i.definitionExpression != null || e && i.floorInfo != null);
    return s.length ? JSON.stringify(s.reduce((i, r) => {
      const o = m(this.floors, r), y = p(o, r.definitionExpression);
      return y != null && (i[r.id] = y), i;
    }, {})) : null;
  }
  get version() {
  ...
  get visibleSublayers() {
    ...
    if (!this.layer)
      return e;
    const s = this.layer.sublayers, n = (r) => {
      const o = this.scale, y = o === 0, h = r.minScale === 0 || o <= r.minScale, c = r.maxScale === 0 || o >= r.maxScale;
      r.visible && (y || h && c) && (r.sublayers ? r.sublayers.forEach(n) : e.unshift(r));
    };
    s && s.forEach(n);
    const i = this._get("visibleSublayers");
    return !i || i.length !== e.length || i.some((r, o) => e[o] !== r) ? e : i;
  }
  toJSON() {
    const e = this.layer;
