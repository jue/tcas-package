import { J as S, K as X, R as H, p as D, l as b, t as w, X as s, A as P, v as T, z as M, U as f } from "./Popup-fkCoWAet.js";
import { u as st, Y as at } from "./Popup-fkCoWAet.js";
import { n as x, o as R } from "./Button-unkVqO-Q.js";
import { s as nt } from "./ColorSet--y3HQ9-1.js";
import "./index-HxXrdrYt.js";
import "vue";
...
class _ extends S {
  ...
  _addOrientationClass() {
    this._settings.themeTags = X(this._settings.themeTags, ["scrollbar", this._settings.orientation]), this._settings.background || (this._settings.background = x.new(this._root, { themeTags: X(this._settings.themeTags, ["main", "background"]) }));
  }
  _makeButton() {
    return this.children.push(R.new(this._root, { themeTags: ["resize", "button", this.get("orientation")], icon: H.new(this._root, { themeTags: ["icon"] }) }));
  }
  _makeThumb() {
    return this.children.push(x.new(this._root, { themeTags: ["thumb", this.get("orientation")] }));
  }
  _handleAnimation(i) {
    i && this._disposers.push(i.events.on("stopped", () => {
      this.setPrivateRaw("isBusy", !1), this._thumbBusy = !1;
    }));
  }
  _afterNew() {
    this._addOrientationClass(), super._afterNew();
    const i = this.startGrip, r = this.endGrip, n = this.thumb, o = this.get("background");
    o && this._disposers.push(o.events.on("click", (a) => {
      this.setPrivateRaw("isBusy", !0);
      const h = this._display.toLocal(a.point), l = this.width(), u = this.height(), m = this.get("orientation");
      let d, g, p;
      d = m == "vertical" ? (h.y - n.height() / 2) / u : (h.x - n.width() / 2) / l, m == "vertical" ? (g = d * u, p = "y") : (g = d * l, p = "x");
      const c = this.get("animationDuration", 0);
      c > 0 ? (this._thumbBusy = !0, this._handleAnimation(this.thumb.animate({ key: p, to: g, duration: c, easing: this.get("animationEasing") }))) : (this.thumb.set(p, g), this._root.events.once("frameended", () => {
        this.setPrivateRaw("isBusy", !1);
      }));
    })), this._disposers.push(n.events.on("dblclick", (a) => {
      if (!D(a.originalEvent, this))
        return;
      const h = this.get("animationDuration", 0), l = this.get("animationEasing");
      this.animate({ key: "start", to: 0, duration: h, easing: l }), this.animate({ key: "end", to: 1, duration: h, easing: l });
    })), this._disposers.push(i.events.on("pointerdown", () => {
      this.setPrivateRaw("isBusy", !0), this._startDown = !0, this._gripDown = "start";
    })), this._disposers.push(r.events.on("pointerdown", () => {
    ...
    })), this._disposers.push(r.events.on("globalpointerup", () => {
      ...
      this._updateThumb();
    })), this._disposers.push(r.on("x", () => {
      this._updateThumb();
    })), this._disposers.push(i.on("y", () => {
      this._updateThumb();
    })), this._disposers.push(r.on("y", () => {
      this._updateThumb();
    })), this._disposers.push(n.events.on("positionchanged", () => {
      this._updateGripsByThumb();
    })), this.get("orientation") == "vertical" ? (i.set("x", 0), r.set("x", 0), this._disposers.push(n.adapters.add("y", (a) => Math.max(Math.min(Number(a), this.height() - n.height()), 0))), this._disposers.push(n.adapters.add("x", (a) => this.width() / 2)), this._disposers.push(i.adapters.add("x", (a) => this.width() / 2)), this._disposers.push(r.adapters.add("x", (a) => this.width() / 2)), this._disposers.push(i.adapters.add("y", (a) => Math.max(Math.min(Number(a), this.height()), 0))), this._disposers.push(r.adapters.add("y", (a) => Math.max(Math.min(Number(a), this.height()), 0)))) : (i.set("y", 0), r.set("y", 0), this._disposers.push(n.adapters.add("x", (a) => Math.max(Math.min(Number(a), this.width() - n.width()), 0))), this._disposers.push(n.adapters.add("y", (a) => this.height() / 2)), this._disposers.push(i.adapters.add("y", (a) => this.height() / 2)), this._disposers.push(r.adapters.add("y", (a) => this.height() / 2)), this._disposers.push(i.adapters.add("x", (a) => Math.max(Math.min(Number(a), this.width()), 0))), this._disposers.push(r.adapters.add("x", (a) => Math.max(Math.min(Number(a), this.width()), 0))));
  }
  _updateChildren() {
  ...
  _changed() {
    if (super._changed(), this.isDirty("start") || this.isDirty("end")) {
      const i = "rangechanged";
      this.events.isEnabled(i) && this.events.dispatch(i, { type: i, target: this, start: this.get("start", 0), end: this.get("end", 1), grip: this._gripDown });
    }
  }
  updateGrips() {
    const i = this.startGrip, r = this.endGrip, n = this.get("orientation"), o = this.height(), a = this.width();
    n == "vertical" ? (i.set("y", o * this.get("start", 0)), r.set("y", o * this.get("end", 1))) : (i.set("x", a * this.get("start", 0)), r.set("x", a * this.get("end", 1)));
    const h = this.getPrivate("positionTextFunction"), l = Math.round(100 * this.get("start", 0)), u = Math.round(100 * this.get("end", 0));
    let m, d;
    h ? (m = h.call(this, this.get("start", 0)), d = h.call(this, this.get("end", 0))) : (m = l + "%", d = u + "%"), i.set("ariaLabel", this._t("From %1", void 0, m)), i.set("ariaValueNow", "" + l), i.set("ariaValueText", l + "%"), i.set("ariaValueMin", "0"), i.set("ariaValueMax", "100"), r.set("ariaLabel", this._t("To %1", void 0, d)), r.set("ariaValueNow", "" + u), r.set("ariaValueText", u + "%"), r.set("ariaValueMin", "0"), r.set("ariaValueMax", "100");
  }
  _updateThumb() {
    const i = this.thumb, r = this.startGrip, n = this.endGrip, o = this.height(), a = this.width();
    let h = r.x(), l = n.x(), u = r.y(), m = n.y(), d = 0, g = 1;
    this.get("orientation") == "vertical" ? b(u) && b(m) && (this._thumbBusy || i.isDragging() || (i.set("height", m - u), i.set("y", u)), d = u / o, g = m / o) : b(h) && b(l) && (this._thumbBusy || i.isDragging() || (i.set("width", l - h), i.set("x", h)), d = h / a, g = l / a), !this.getPrivate("isBusy") || this.get("start") == d && this.get("end") == g || (this.set("start", d), this.set("end", g));
    const p = this.getPrivate("positionTextFunction"), c = Math.round(100 * this.get("start", 0)), L = Math.round(100 * this.get("end", 0));
    let v, y;
    p ? (v = p.call(this, this.get("start", 0)), y = p.call(this, this.get("end", 0))) : (v = c + "%", y = L + "%"), i.set("ariaLabel", this._t("From %1 to %2", void 0, v, y)), i.set("ariaValueNow", "" + c), i.set("ariaValueText", c + "%");
  }
  _updateGripsByThumb() {
    const i = this.thumb, r = this.startGrip, n = this.endGrip;
    if (this.get("orientation") == "vertical") {
      const o = i.height();
      r.set("y", i.y()), n.set("y", i.y() + o);
    } else {
      const o = i.width();
      r.set("x", i.x()), n.set("x", i.x() + o);
    }
  }
}
Object.defineProperty(_, "className", { enumerable: !0, configurable: !0, writable: !0, value: "Scrollbar" }), Object.defineProperty(_, "classNames", { enumerable: !0, configurable: !0, writable: !0, value: S.classNames.concat([_.className]) });
...
let e = class extends w {
  constructor() {
    super(...arguments), Object.defineProperty(this, "responsiveRules", { enumerable: !0, configurable: !0, writable: !0, value: [] });
  }
  static widthXXS(t, i) {
    return t <= e.XXS;
  }
...
const tt = e;
export {
  Q as AnimatedThemeAm5,
  nt as ColorSetAm5,
  Z as DarkThemeAm5,
  tt as ResponsiveThemeAm5,
  _ as ScrollbarAm5,
  w as ThemeAm5,
  st as TooltipAm5,
  at as colorAm5
