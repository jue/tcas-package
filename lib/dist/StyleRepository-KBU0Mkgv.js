import { a as T, f as $, l as Q } from "./StyleDefinition-DRnpTfyN.js";
import { L as W } from "./enums-An2lvwTb.js";
import { C as g } from "./enums-Vyj7xNgv.js";
import { t as A } from "./VertexElementDescriptor-L2lGUBx9.js";
import { fD as b, j3 as fe } from "./index-HxXrdrYt.js";
import { v as Jt, p as Xt, z as te } from "./colorUtils-lH7-fxuz.js";
import { h as P, t as vt } from "./GeometryUtils-UnkSlEkY.js";
import { $ as ge } from "./definitions-5wPyT42Z.js";
let _e = class {
  constructor(t) {
  ...
  static i8888to32(t, e, r, s) {
    return 255 & t | (255 & e) << 8 | (255 & r) << 16 | s << 24;
  }
...
let H = class Dt {
  ...
  setDataUniforms(t, e, r, s, i) {
    const o = this.getUniforms();
    for (const n of o) {
      const { name: l, type: u, getValue: p } = n, h = p(r, e, s, i);
      if (h !== null)
        switch (u) {
          case "float":
            t.setUniform1f(l, h);
  ...
  encodeAttributes(t, e, r, s) {
    var p;
    const i = this.attributesInfo(), o = this.getEncodingInfos(), n = [];
    let l = 0, u = 0;
    for (const h of Object.keys(o)) {
      const f = o[h], { type: _, precisionFactor: d, isLayout: O } = i[h], R = O ? r.getLayoutProperty(h) : r.getPaintProperty(h), M = (p = R.interpolator) == null ? void 0 : p.getInterpolationRange(e);
      let I = 0;
      for (const x of f) {
        ...
        const v = s ?? R.getValue(M ? M[I] : e, t);
        switch (_) {
          case c.R8_SIGNED:
          case c.R8_UNSIGNED:
            n[l] |= this._encodeByte(v * (d || 1), 8 * E);
            break;
          case c.R16_SIGNED:
          case c.R16_UNSIGNED:
            n[l] |= this._encodeShort(v * (d || 1), 8 * E);
            break;
          case c.R8G8_SIGNED:
          case c.R8G8_UNSIGNED:
            n[l] |= this._encodeByte(v * (d || 1), 8 * E), n[l] |= this._encodeByte(v * (d || 1), 8 * E + 8);
            break;
          case c.R16G16_SIGNED:
          case c.R16G16_UNSIGNED:
            n[l] |= this._encodeShort(v * (d || 1), 8 * E), n[l] |= this._encodeShort(v * (d || 1), 8 * E + 16);
            break;
          case c.R8G8B8A8_SIGNED:
          case c.R8G8B8A8_UNSIGNED:
            n[l] |= this._encodeByte(v * (d || 1), 8 * E), n[l] |= this._encodeByte(v * (d || 1), 8 * E + 8), n[l] |= this._encodeByte(v * (d || 1), 8 * E + 16), n[l] |= this._encodeByte(v * (d || 1), 8 * E + 24);
            break;
  ...
  _buildAttributesInfo() {
    ...
    for (const h of o) {
      ...
      if (f === K.UNIFORM || f === K.UNUSED)
        continue;
      const _ = i[h], d = [];
      e[h] = d;
      const O = _.type;
      for (let R = 0; R < f; R++) {
        const { dataType: M, bytesPerElement: I, count: x, normalized: E } = Dt._encodingInfo[O], G = I * x, v = `${M}-${E === !0}`;
        let D = r[v], k = 0;
        if (!D || D.count + x > 4)
          s++, D = { dataIndex: s, count: 0, offset: 0 }, x !== 4 && (r[v] = D), t.push({ location: -1, name: "a_data_" + s, count: x, type: M, normalized: E }), k = Math.ceil(Math.max(G / 4, 1));
        else {
          const z = t[D.dataIndex];
          z.count += x, k = Math.ceil(Math.max(z.count * I / 4, 1)) - Math.ceil(Math.max(D.offset / 4, 1));
        }
        d.push({ dataIndex: D.dataIndex, offset: D.offset, bufferElementsToAdd: k }), D.offset += G, D.count += x;
      }
    }
    for (const h of t)
      switch (h.type) {
  ...
  _buildVertexBufferLayout(t) {
    ...
    if (t.length === 0)
      ...
      for (const n of t)
        s += ee(n.type) * n.count;
      for (const n of r)
        i.push(new A(n.name, n.count, n.type, n.offset, s, n.normalized));
      for (const n of t)
        i.push(new A(n.name, n.count, n.type, o, s, n.normalized)), o += ee(n.type) * n.count;
      e.geometry = i;
    }
...
`;
    ...
    for (const u of o) {
      l++;
      const { name: p, type: h, precisionFactor: f, isLayout: _ } = n[u], d = f && f !== 1 ? " * " + 1 / f : "", { bytesPerElement: O, count: R } = Dt._encodingInfo[h], M = (I) => `a_data_${I.dataIndex}${de(R, I.offset, O)}`;
      switch (this.getAtributeState(l)) {
        case K.UNIFORM:
          {
            const I = this._getType(R), x = `u_${p}`;
            i.push({ name: x, type: I, getValue: (E, G, v, D) => {
              const k = _ ? E.getLayoutValue(u, G) : E.getPaintValue(u, G);
              if (h === c.R16G16B16A16_DASHARRAY) {
                const z = E.getDashKey(k, E.getLayoutValue("line-cap", G)), it = D.getMosaicItemPosition(z, !1);
                if (it == null)
                  return null;
                const { tl: at, br: Qt } = it;
                return [at[0], Qt[1], Qt[0], at[1]];
              }
...
`;
        ...
        case K.DATA_DRIVEN:
          {
            const I = M(e[u][0]);
            s += `${this._getType(R)} ${p} = ${I}${d};
`;
          }
          break;
        case K.INTERPOLATED_DATA_DRIVEN: {
          const I = `u_t_${p}`;
          i.push({ name: I, type: "float", getValue: (G, v, D, k) => (_ ? G.getLayoutProperty(u) : G.getPaintProperty(u)).interpolator.interpolationUniformValue(D, v) }), r += `uniform float ${I};
`;
          const x = M(e[u][0]), E = M(e[u][1]);
          s += `${this._getType(R)} ${p} = mix(${x}${d}, ${E}${d}, ${I});
`;
        }
      }
    }
    this._shaderHeader = r, this._shaderMain = s, this._uniforms = i;
  }
  _bit(t) {
  ...
  _encodeColor(t) {
    const e = 255 * t[3];
    return _e.i8888to32(t[0] * e, t[1] * e, t[2] * e, e);
  }
  _encodePattern(t, e, r) {
    if (!r || !r.rect)
      return;
    const s = 2, i = r.rect, o = r.width, n = r.height;
    e[t] = this._encodeShort(i.x + s, 0), e[t] |= this._encodeShort(i.y + s + n, 16), e[t + 1] = this._encodeShort(i.x + s + o, 0), e[t + 1] |= this._encodeShort(i.y + s, 16);
  }
  _encodeByte(t, e) {
    return (255 & t) << e;
  }
...
J.ATTRIBUTES = [], J.GEOMETRY_LAYOUT = [new A("a_pos", 2, g.BYTE, 0, 2)], J.ATTRIBUTES_INFO = {};
...
const oe = [new A("a_pos", 2, g.SHORT, 0, 16), new A("a_vertexOffset", 2, g.SHORT, 4, 16), new A("a_texAngleRange", 4, g.UNSIGNED_BYTE, 8, 16), new A("a_levelInfo", 4, g.UNSIGNED_BYTE, 12, 16)], le = [new A("a_opacityInfo", 1, g.UNSIGNED_BYTE, 0, 1)];
let ot = class Rt extends H {
  constructor(t) {
    super(t);
  }
  geometryInfo() {
...
const ue = { kind: "null" }, w = { kind: "number" }, B = { kind: "string" }, N = { kind: "boolean" }, Y = { kind: "color" }, Tt = { kind: "object" }, C = { kind: "value" };
function ct(a, t) {
  return { kind: "array", itemType: a, n: t };
}
const me = [ue, w, B, N, Y, Tt, ct(C)];
function rt(a) {
  if (a.kind === "array") {
    const t = rt(a.itemType);
...
function bt(a) {
  ...
  if (Array.isArray(a)) {
    ...
    return ct(t || C, a.length);
  }
  return typeof a == "object" ? Tt : C;
}
function Nt(a, t) {
  if (t.kind === "array")
    return a.kind === "array" && (a.n === 0 && a.itemType.kind === "value" || Nt(a.itemType, t.itemType)) && (typeof t.n != "number" || t.n === a.n);
  if (t.kind === "value") {
    for (const e of me)
      if (Nt(a, e))
        return !0;
...
function xt(a) {
  if (a === null)
    return "";
  const t = typeof a;
  return t === "string" ? a : t === "number" || t === "boolean" ? String(a) : a instanceof b ? a.toString() : JSON.stringify(a);
}
let we = class {
  constructor(t) {
...
class U {
  constructor(t, e, r) {
    this._lhs = t, this._rhs = e, this._compare = r, this.type = N;
  }
  static parse(t, e, r) {
...
class kt {
  ...
  static parse(t, e, r) {
    ...
    for (let n = 1; n < t.length - 1; n += 2) {
      ...
      s || (s = u.type), i.push({ condition: l, output: u });
    }
    const o = y(t[t.length - 1], e, r);
    return s || (s = o.type), new kt(s, i, o);
  }
  evaluate(t, e) {
    for (const r of this._args)
...
class zt {
  constructor(t, e, r, s, i) {
    this.type = t, this._input = e, this._labels = r, this._outputs = s, this._fallback = i;
  }
  static parse(t, e) {
    if (t.length < 3)
    ...
    return new zt(r, s, o, i, y(t[t.length - 1], e));
  }
  evaluate(t, e) {
...
let _t = class j {
  constructor(t, e, r, s, i) {
    this._operator = t, this.type = e, this.interpolation = r, this.input = s, this._stops = i;
  }
  static parse(t, e, r) {
    const s = t[0];
    if (t.length < 5)
    ...
    const o = y(t[2], e, w);
    let n;
    s === "interpolate-hcl" || s === "interpolate-lab" ? n = Y : r && r.kind !== "value" && (n = r);
    const l = [];
    ...
    if (n && n !== Y && n !== w && (n.kind !== "array" || n.itemType !== w))
      throw new Error(`"${s}" cannot interpolate type ${rt(n)}`);
    return new j(s, n, i, o, l);
  }
  evaluate(t, e) {
    const r = this._stops;
    if (r.length === 1)
    ...
    let i = 0;
    for (; ++i < r.length && !(s < r[i][0]); )
      ;
    const o = r[i - 1][0], n = r[i][0], l = j.interpolationRatio(this.interpolation, s, o, n), u = r[i - 1][1].evaluate(t, e), p = r[i][1].evaluate(t, e);
    if (this._operator === "interpolate") {
      ...
      if (this.type.kind === "color" && u instanceof b && p instanceof b) {
        const h = new b(u), f = new b(p);
        return new b([P(h.r, f.r, l), P(h.g, f.g, l), P(h.b, f.b, l), P(h.a, f.a, l)]);
      }
    ...
    if (this._operator === "interpolate-hcl") {
      const h = Jt(u), f = Jt(p), _ = f.h - h.h, d = Xt({ h: h.h + l * (_ > 180 || _ < -180 ? _ - 360 * Math.round(_ / 360) : _), c: P(h.c, f.c, l), l: P(h.l, f.l, l) });
      return new b({ a: P(u.a, p.a, l), ...d });
    }
    if (this._operator === "interpolate-lab") {
      const h = te(u), f = te(p), _ = Xt({ l: P(h.l, f.l, l), a: P(h.a, f.a, l), b: P(h.b, f.b, l) });
      return new b({ a: P(u.a, p.a, l), ..._ });
    }
  ...
  interpolationUniformValue(t, e) {
    ...
    for (; ++s < r.length && !(t < r[s][0]); )
      ;
    const i = r[s - 1][0], o = r[s][0];
    return j.interpolationRatio(this.interpolation, e, i, o);
  }
  ...
  static interpolationRatio(t, e, r, s) {
    let i = 0;
    return t[0] === "linear" ? i = j._exponentialInterpolationRatio(e, 1, r, s) : t[0] === "exponential" ? i = j._exponentialInterpolationRatio(e, t[1], r, s) : t[0] === "cubic-bezier" && (i = fe(t[1], t[2], t[3], t[4])(j._exponentialInterpolationRatio(e, 1, r, s), 1e-5)), i < 0 ? i = 0 : i > 1 && (i = 1), i;
  }
...
class $t {
  ...
  static parse(t, e, r) {
    ...
    const i = y(t[t.length - 1], s, r);
    return new $t(i.type, i);
  }
  evaluate(t, e) {
...
class Vt {
  ...
  static parse(t, e) {
    ...
    const r = y(t[1], e, w), s = y(t[2], e);
    return new Vt(s.type.itemType, r, s);
  }
  evaluate(t, e) {
...
class dt {
  ...
  static parse(t, e) {
    ...
    switch (t.length) {
      ...
      case 3:
        return r = y(t[1], e), s = y(t[2], e), new dt(r, s);
      default:
...
class mt {
  constructor(t, e) {
    this._key = t, this._obj = e, this.type = N;
  }
  static parse(t, e) {
    let r, s;
    switch (t.length) {
      ...
      case 3:
        return r = y(t[1], e), s = y(t[2], e), new mt(r, s);
      default:
...
class Mt {
  constructor(t, e) {
    this._key = t, this._vals = e, this.type = N;
  }
  static parse(t, e) {
    if (t.length !== 3)
      throw new Error('"in" expects 2 arguments');
    return new Mt(y(t[1], e), y(t[2], e));
  }
  evaluate(t, e) {
    const r = this._key.evaluate(t, e);
...
class wt {
  ...
  static parse(t, e) {
    ...
    if (t.length === 4) {
      ...
      if (i.type !== w)
        throw new Error('"slice" index must return a number');
      return new wt(r.type, r, s, i);
    }
    return new wt(r.type, r, s);
  }
  evaluate(t, e) {
...
class S {
  ...
  static parse(t, e, r) {
    const s = t.slice(1).map((i) => y(i, e));
    return new S(s, r);
  }
  evaluate(t, e) {
    let r;
    return this._args && (r = this._args.map((s) => s.evaluate(t, e))), this._calculate(r);
  }
}
class Pe extends S {
  static parse(t, e) {
...
st.ops = { downcase: (a) => a.toLowerCase(), upcase: (a) => a.toUpperCase() };
class jt {
  constructor(t) {
  ...
  evaluate(t, e) {
    const r = this._validate(this._args[0].evaluate(t, e)), s = this._validate(this._args[1].evaluate(t, e)), i = this._validate(this._args[2].evaluate(t, e));
    return new b({ r, g: s, b: i });
  }
  _validate(t) {
...
class Kt {
  ...
  evaluate(t, e) {
    const r = this._validate(this._args[0].evaluate(t, e)), s = this._validate(this._args[1].evaluate(t, e)), i = this._validate(this._args[2].evaluate(t, e)), o = this._validateAlpha(this._args[3].evaluate(t, e));
    return new b({ r, g: s, b: i, a: o });
  }
  _validate(t) {
    if (typeof t != "number" || t < 0 || t > 255)
      throw new Error(`${t}: invalid color component`);
...
class q {
  ...
  evaluate(t, e) {
    ...
    for (const s of this._args) {
      const i = s.evaluate(t, e);
      if (r = bt(i), Nt(r, this.type))
        return i;
    }
...
class F {
  ...
  evaluate(t, e) {
    ...
    if (this.type === B)
      return xt(this._args[0].evaluate(t, e));
    if (this.type === w) {
      for (const r of this._args) {
        const s = Number(r.evaluate(t, e));
        if (!isNaN(s))
          return s;
      }
      return null;
    ...
    if (this.type === Y) {
      for (const r of this._args)
        try {
          const s = F.toColor(r.evaluate(t, e));
          if (s instanceof b)
            return s;
        } catch {
        }
...
F.types = { "to-boolean": N, "to-color": Y, "to-number": w, "to-string": B };
class Wt {
  constructor(t) {
    this._val = t, this.type = bt(t);
  }
  static parse(t) {
    if (t.length !== 2)
...
class Zt {
  ...
  evaluate(t, e) {
    return rt(bt(this._arg.evaluate(t, e)));
  }
}
function y(a, t, e) {
  const r = typeof a;
  if (r === "string" || r === "boolean" || r === "number" || a === null) {
    if (e)
      switch (e.kind) {
        case "string":
          r !== "string" && (a = F.toString(a));
          break;
        case "number":
          r !== "number" && (a = F.toNumber(a));
          break;
        case "color":
          a = F.toColor(a);
      }
  ...
  const i = ye[s];
  if (i === void 0)
    throw new Error(`Invalid expression operator "${s}"`);
  if (!i)
    throw new Error(`Unimplemented expression operator "${s}"`);
  return i.parse(a, t, e);
}
const ye = { array: q, boolean: q, collator: null, format: null, image: null, literal: Wt, number: q, "number-format": null, object: q, string: q, "to-boolean": F, "to-color": F, "to-number": F, "to-string": F, typeof: Zt, accumulated: null, "feature-state": null, "geometry-type": Lt, id: St, "line-progress": null, properties: Ee, at: Vt, get: dt, has: mt, in: Mt, "index-of": xe, length: Ct, slice: wt, "!": Ut, "!=": be, "<": Ie, "<=": ve, "==": Te, ">": De, ">=": Re, all: Bt, any: Ot, case: kt, coalesce: Ae, match: zt, within: null, interpolate: _t, "interpolate-hcl": _t, "interpolate-lab": _t, step: Ft, let: $t, var: Ne, concat: Ht, downcase: st, "is-supported-script": null, "resolved-locale": null, upcase: st, rgb: jt, rgba: Kt, "to-rgba": qt, "-": Pe, "*": Se, "/": Le, "%": Ue, "^": Be, "+": Oe, abs: m, acos: m, asin: m, atan: m, ceil: m, cos: m, e: m, floor: m, ln: m, ln2: m, log10: m, log2: m, max: m, min: m, pi: m, round: m, sin: m, sqrt: m, tan: m, zoom: It, "heatmap-density": null, "has-id": Yt, none: Gt };
class L {
  constructor(t) {
    this._expression = t;
  }
...
class Et {
  constructor(t, e) {
    ...
    if (e == null && (e = t.default), Array.isArray(e) && e.length > 0 && ye[e[0]]) {
      const i = { number: w, color: Y, string: B, boolean: N, enum: B };
      try {
        const o = t.type === "array" ? ct(i[t.value] || C, t.length) : i[t.type], n = y(e, null, o);
        this.getValue = this._buildExpression(n, t), this.isDataDriven = !0, n instanceof _t && n.input instanceof It && (this.interpolator = n);
      } catch (o) {
    ...
    if (this.isDataDriven = !!e && !!e.property, this.isDataDriven)
      ...
      s ? this.getValue = r ? this._buildZoomInterpolate(e) : this._buildZoomInterval(e) : (e = this._validate(e, t), this.getValue = this._buildSimple(e));
  }
  _validate(t, e) {
    if (e.type === "number") {
      ...
      e.type === "color" ? t = Et._parseColor(t) : e.type === "enum" ? typeof t == "string" && (t = e.values.indexOf(t)) : e.type === "array" && e.value === "enum" ? t = t.map((r) => typeof r == "string" ? e.values.indexOf(r) : r) : e.type === "string" && (t = xt(t));
    return t;
  }
  _buildSimple(t) {
    return () => t;
  ...
  _buildIdentity(t, e) {
    return (r, s) => {
      let i;
      return s && (i = s.values[t.property]), i !== void 0 && (i = this._validate(i, e)), i ?? (t.default !== void 0 ? t.default : e.default);
    };
  }
  _buildCategorical(t, e) {
    return (r, s) => {
      let i;
      return s && (i = s.values[t.property]), i = this._categorical(i, t.stops), i !== void 0 ? i : t.default !== void 0 ? t.default : e.default;
    };
  }
  _buildInterval(t, e) {
    return (r, s) => {
      let i;
      return s && (i = s.values[t.property]), typeof i == "number" ? this._interval(i, t.stops) : t.default !== void 0 ? t.default : e.default;
    };
  }
  _buildInterpolate(t, e) {
    return (r, s) => {
      let i;
      return s && (i = s.values[t.property]), typeof i == "number" ? this._interpolate(i, t.stops, t.base || 1) : t.default !== void 0 ? t.default : e.default;
    };
  }
  ...
  _interpolate(t, e, r) {
    ...
    if (s && i) {
      const n = i[0] - s[0], l = t - s[0], u = r === 1 ? l / n : (r ** l - 1) / (r ** n - 1);
...
var ut;
(function(a) {
  a[a.BUTT = 0] = "BUTT", a[a.ROUND = 1] = "ROUND", a[a.SQUARE = 2] = "SQUARE", a[a.UNKNOWN = 4] = "UNKNOWN";
})(ut || (ut = {}));
class ht {
  constructor(t, e, r, s) {
    switch (this.type = t, this.typeName = e.type, this.id = e.id, this.source = e.source, this.sourceLayer = e["source-layer"], this.minzoom = e.minzoom, this.maxzoom = e.maxzoom, this.filter = e.filter, this.layout = e.layout, this.paint = e.paint, this.z = r, this.uid = s, t) {
      case T.BACKGROUND:
  ...
  getLayoutValue(t, e, r) {
    ...
    return i && (s = i.getValue(e, r)), s === void 0 && (s = this._layoutDefinition[t].default), s;
  }
  getPaintValue(t, e, r) {
    let s;
    const i = this._paintProperties[t];
    return i && (s = i.getValue(e, r)), s === void 0 && (s = this._paintDefinition[t].default), s;
  }
  isPainterDataDriven() {
  ...
  _parseLayout(t) {
    const e = {};
    for (const r in t) {
      const s = this._layoutDefinition[r];
      s && (e[r] = new Et(s, t[r]));
    }
    return e;
  }
  _parsePaint(t) {
    const e = {};
    for (const r in t) {
      const s = this._paintDefinition[r];
      s && (e[r] = new Et(s, t[r]));
    }
    return e;
  }
  computeAttributesKey(t, e, r, s) {
    let i = 0, o = 0;
    for (const n of e) {
      let l = 3;
      if (!n || n !== s) {
        const u = r[n], { isLayout: p, isOptional: h } = u, f = p ? this.getLayoutProperty(n) : this.getPaintProperty(n);
...
class re extends ht {
  constructor(t, e, r, s) {
    super(t, e, r, s), this.backgroundMaterial = new J(this.computeAttributesKey(W.BACKGROUND, J.ATTRIBUTES, J.ATTRIBUTES_INFO));
  }
}
class se extends ht {
  constructor(t, e, r, s) {
...
class ae extends ht {
  constructor(t, e, r, s) {
    var i, o, n, l, u, p, h, f, _, d, O, R;
    super(t, e, r, s), this.iconMaterial = new ot(this.computeAttributesKey(W.ICON, ot.ATTRIBUTES, ot.ATTRIBUTES_INFO)), this.textMaterial = new lt(this.computeAttributesKey(W.TEXT, lt.ATTRIBUTES, lt.ATTRIBUTES_INFO)), this.hasDataDrivenIcon = ((i = this.getPaintProperty("icon-color")) == null ? void 0 : i.isDataDriven) || ((o = this.getPaintProperty("icon-halo-blur")) == null ? void 0 : o.isDataDriven) || ((n = this.getPaintProperty("icon-halo-color")) == null ? void 0 : n.isDataDriven) || ((l = this.getPaintProperty("icon-halo-width")) == null ? void 0 : l.isDataDriven) || ((u = this.getPaintProperty("icon-opacity")) == null ? void 0 : u.isDataDriven) || ((p = this.getLayoutProperty("icon-size")) == null ? void 0 : p.isDataDriven), this.hasDataDrivenText = ((h = this.getPaintProperty("text-color")) == null ? void 0 : h.isDataDriven) || ((f = this.getPaintProperty("text-halo-blur")) == null ? void 0 : f.isDataDriven) || ((_ = this.getPaintProperty("text-halo-color")) == null ? void 0 : _.isDataDriven) || ((d = this.getPaintProperty("text-halo-width")) == null ? void 0 : d.isDataDriven) || ((O = this.getPaintProperty("text-opacity")) == null ? void 0 : O.isDataDriven) || ((R = this.getLayoutProperty("text-size")) == null ? void 0 : R.isDataDriven);
  }
}
class ne extends ht {
  constructor(t, e, r, s) {
    super(t, e, r, s), this.circleMaterial = new X(this.computeAttributesKey(W.CIRCLE, X.ATTRIBUTES, X.ATTRIBUTES_INFO));
  }
}
class je {
  constructor(t, e, r) {
    let s;
    this.allowOverlap = t.getLayoutValue("icon-allow-overlap", e), this.ignorePlacement = t.getLayoutValue("icon-ignore-placement", e), this.keepUpright = t.getLayoutValue("icon-keep-upright", e), this.optional = t.getLayoutValue("icon-optional", e), this.rotationAlignment = t.getLayoutValue("icon-rotation-alignment", e), this.rotationAlignment === Q.AUTO && (this.rotationAlignment = r ? Q.MAP : Q.VIEWPORT), s = t.getLayoutProperty("icon-anchor"), s != null && s.isDataDriven ? this._anchorProp = s : this.anchor = t.getLayoutValue("icon-anchor", e), s = t.getLayoutProperty("icon-offset"), s != null && s.isDataDriven ? this._offsetProp = s : this.offset = t.getLayoutValue("icon-offset", e), s = t.getLayoutProperty("icon-padding"), s != null && s.isDataDriven ? this._paddingProp = s : this.padding = t.getLayoutValue("icon-padding", e), s = t.getLayoutProperty("icon-rotate"), s != null && s.isDataDriven ? this._rotateProp = s : this.rotate = t.getLayoutValue("icon-rotate", e), s = t.getLayoutProperty("icon-size"), s != null && s.isDataDriven ? this._sizeProp = s : this.size = t.getLayoutValue("icon-size", e);
  }
  update(t, e) {
...
class nt {
  constructor(t) {
    if (this._style = t, this.backgroundBucketIds = [], this._uidToLayer = /* @__PURE__ */ new Map(), this._layerByName = {}, this._runningId = 0, t.layers || (t.layers = []), this.version = parseFloat(t.version), this.layers = t.layers.map((e, r, s) => this._create(e, r, s)).filter((e) => !!e), this.layers) {
      let e;
      for (let r = 0; r < this.layers.length; r++)
        e = this.layers[r], this._layerByName[e.id] = e, this._uidToLayer.set(e.uid, e), e.type === T.BACKGROUND && this.backgroundBucketIds.push(e.id);
    }
  ...
  setStyleLayer(t, e) {
    ...
    if (o) {
      ...
      e || (e = n), e === n ? (i = !1, s = nt._recreateLayer(t, o), this.layers[e] = s, r.layers[e] = t) : (this.layers.splice(n, 1), r.layers.splice(n, 1), s = this._create(t, e, this.layers), this.layers.splice(e, 0, s), r.layers.splice(e, 0, t));
    } else
      s = this._create(t, e, this.layers), !e || e >= this.layers.length ? (this.layers.push(s), r.layers.push(t)) : (this.layers.splice(e, 0, s), r.layers.splice(e, 0, t));
    this._layerByName[t.id] = s, this._uidToLayer.set(s.uid, s), i && this._recomputeZValues(), this._identifyRefLayers();
  }
  getStyleLayer(t) {
    const e = this._layerByName[t];
  ...
  setPaintProperties(t, e) {
    ...
    if (!r)
      return;
    const s = { type: r.typeName, id: r.id, source: r.source, "source-layer": r.sourceLayer, minzoom: r.minzoom, maxzoom: r.maxzoom, filter: r.filter, layout: r.layout, paint: e }, i = nt._recreateLayer(s, r), o = this.layers.indexOf(r);
  ...
  setLayoutProperties(t, e) {
    ...
    if (!r)
      return;
    const s = { type: r.typeName, id: r.id, source: r.source, "source-layer": r.sourceLayer, minzoom: r.minzoom, maxzoom: r.maxzoom, filter: r.filter, layout: e, paint: r.paint }, i = nt._recreateLayer(s, r), o = this.layers.indexOf(r);
  ...
  setStyleLayerVisibility(t, e) {
    ...
    const s = r.layout || {};
    s.visibility = e;
    const i = { type: r.typeName, id: r.id, source: r.source, "source-layer": r.sourceLayer, minzoom: r.minzoom, maxzoom: r.maxzoom, filter: r.filter, layout: s, paint: r.paint }, o = nt._recreateLayer(i, r), n = this.layers.indexOf(r);
    this.layers[n] = o, this._style.layers[n].layout = s, this._layerByName[r.id] = o, this._uidToLayer.set(r.uid, o);
  }
  ...
  _identifyRefLayers() {
    ...
    for (const s of this.layers) {
      ...
      if (s.type === T.FILL) {
        ...
        n += "|" + ((i == null ? void 0 : i.visibility) ?? ""), n += "|" + s.minzoom, n += "|" + s.maxzoom, n += "|" + JSON.stringify(s.filter), (o.hasDataDrivenFill || o.hasDataDrivenOutline) && (n += "|" + r), t.push({ key: n, layer: s });
      } else if (s.type === T.LINE) {
        const o = s, n = s.paint, l = n != null && (n["line-pattern"] != null || n["line-dasharray"] != null);
        let u = s.source + "|" + s.sourceLayer;
        u += "|" + ((i == null ? void 0 : i.visibility) ?? ""), u += "|" + s.minzoom, u += "|" + s.maxzoom, u += "|" + JSON.stringify(s.filter), u += "|" + (i !== void 0 ? i["line-cap"] : ""), u += "|" + (i !== void 0 ? i["line-join"] : ""), (o.hasDataDrivenLine || l) && (u += "|" + r), e.push({ key: u, layer: s });
      }
      ++r;
    }
  ...
  _assignRefLayers(t) {
    ...
    for (let i = 0; i < s; i++) {
      ...
      else if (e = o.key, r = o.layer.id, o.layer.type === T.FILL) {
  ...
  _create(t, e, r) {
    const s = 1 - (1 + e) * (1 / (r.length + 1)), i = this._runningId++;
    switch (t.type) {
      case "background":
        return new re(T.BACKGROUND, t, s, i);
      case "fill":
        return new se(T.FILL, t, s, i);
      case "line":
        return new ie(T.LINE, t, s, i);
      case "symbol":
        return new ae(T.SYMBOL, t, s, i);
      case "raster":
        return console.warn(`Unsupported vector tile raster layer ${t.id}`), null;
      case "circle":
        return new ne(T.CIRCLE, t, s, i);
    }
    return null;
  }
  static _recreateLayer(t, e) {
    switch (t.type) {
      case "background":
        return new re(T.BACKGROUND, t, e.z, e.uid);
      case "fill":
        return new se(T.FILL, t, e.z, e.uid);
      case "line":
        return new ie(T.LINE, t, e.z, e.uid);
      case "symbol":
        return new ae(T.SYMBOL, t, e.z, e.uid);
      case "raster":
        return console.warn(`Unsupported vector tile raster layer ${t.id}`), null;
      case "circle":
        return new ne(T.CIRCLE, t, e.z, e.uid);
    }
    return null;
  }
}
