import { ck as f, co as j, cl as x, bm as b, a9 as y, aC as B, _ as U, ae as h, H as u, a as r, b as s, c as T, eM as k, ax as P, C as R, cm as $, cp as _, az as I, d4 as a } from "./index-HxXrdrYt.js";
import { o as M } from "./imageBitmapUtils-FC9sb4eL.js";
import "vue";
var v;
const c = new k("0/0/0", 0, 0, 0, void 0);
let g = v = class extends f(j(x(P))) {
  constructor() {
    super(...arguments), this.tileInfo = b.create({ spatialReference: y.WebMercator, size: 256 }), this.type = "base-tile", this.fullExtent = new B(-20037508342787e-6, -2003750834278e-5, 2003750834278e-5, 20037508342787e-6, y.WebMercator), this.spatialReference = y.WebMercator;
  }
  getTileBounds(e, t, i, l) {
    const n = l || U();
    return c.level = e, c.row = t, c.col = i, c.extent = n, this.tileInfo.updateTileInfo(c), c.extent = void 0, n;
  }
  fetchTile(e, t, i, l = {}) {
    const { signal: n } = l, p = this.getTileUrl(e, t, i), d = { responseType: "image", signal: n, query: { ...this.refreshParameters } };
    return h(p ?? "", d).then((m) => m.data);
  }
  async fetchImageBitmapTile(e, t, i, l = {}) {
    ...
    const p = this.getTileUrl(e, t, i) ?? "", d = { responseType: "blob", signal: n, query: { ...this.refreshParameters } }, { data: m } = await h(p, d);
    return M(m, e, t, i, n);
  }
  getTileUrl() {
...
let o = class extends f($(_(L))) {
  ...
  getTileUrl(e, t, i) {
    if (!this.loaded || this.bingMetadata == null)
      return null;
  ...
  _getMetadata() {
    ...
    return h(`${O}/REST/v1/Imagery/Metadata/${e}`, { responseType: "json", query: { include: "ImageryProviders", uriScheme: "https", key: this.key, suppressStatus: !0, output: "json", culture: this.culture, userRegion: this.region } }).then((t) => {
      const i = t.data;
      if (i.statusCode !== 200)
        throw new u("bingmapslayer:getmetadata", i.statusDescription);
      if (this.bingMetadata = i, this.bingMetadata.resourceSets.length === 0)
        throw new u("bingmapslayer:getmetadata", "no bing resourcesets");
      if (this.bingMetadata.resourceSets[0].resources.length === 0)
        throw new u("bingmapslayer:getmetadata", "no bing resources");
    }).catch((t) => {
  ...
  _getPortalBingKey() {
    return h(this.portalUrl ?? "", { responseType: "json", authMode: "no-prompt", query: { f: "json" } }).then((e) => {
      if (!e.data.bingKey)
        throw new u("bingmapslayer:getportalbingkey", "The referenced Portal does not contain a valid bing key");
